...
class Water extends THREE.Mesh {
    constructor(geometry, options = {}) {
        ...
        const mirrorShader = {
            ...
            fragmentShader: /* glsl */`
                ...
                vec4 getNoise( vec2 uv ) {
                    ...
                    vec2 uv3 = uv / vec2( 1091.0, 1021.0 ) - vec2( time / 103.0, time / 89.0 );
                ...
                void main() {
                    ...
                    vec3 eyeDirection = normalize( eye - worldPosition.xyz );
                    vec3 diffuseColor = vec3(0.0);
                    vec3 specularColor = vec3(0.0);

                    sunLight( surfaceNormal, eyeDirection, 100.0, 2.0, 0.5, diffuseColor, specularColor );

                    vec3 nonreflectingNormal = surfaceNormal;

                    vec4 mirror = texture2DProj( mirrorSampler, mirrorCoord );

                    vec3 color = alpha * mirror.rgb + ( 1.0 - alpha ) * ( diffuseColor * waterColor + specularColor );
        ...
        scope.onBeforeRender = function (renderer, scene, camera) {
            ...
            renderer.state.buffers.depth.setMask(true); // make sure the depth buffer is writable so it can be cleared by renderer.clear()
